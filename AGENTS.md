...
- **Compose CI:** run `stacks/scripts/compose-validate.sh` from any cwd (script locates repo root via `HIVE_OBJECTIVE.md`).
...
- **Audit (2026-04-30):** **`stacks/docs` → `../docs` must not exist**; **`stacks/._DAV`** is SMB/Finder noise—delete when idle, never commit. **`.ruff_cache/`** is gitignored. **Compose:** `stacks/scripts/compose-validate.sh` passes. **Git / `inventory.py`:** if `git pull`/`git merge` fails with **Resource busy** or **EINVAL** on `docs/hive/tools/inventory.py`, close editor handles and retry, or merge from an **APFS git worktree** (`git worktree add -b <tmp-branch> <apfs-path> main` → `git merge origin/main` → `git push origin HEAD:main`). **`origin/main`:** merge `e15c4bd` (includes `1c774b3` — `out_dir` under `repo_root/docs/hive/proposals/…`, not `stacks/…/docs/…`).
...
### Baseline Status — 12 Managed Stacks
...
| ollama      | ✓            | ✓                            | ✓                    | ✓         | ✓         | ✓                   | semver pinned; open-webui depends_on without condition | ✓      | ✓            | ✓            |
...
- [2026-04-30] **ollama/open-webui**: `depends_on: - ollama` without `condition: service_healthy` is a minor race risk. Low priority since ollama publishes a health endpoint and start_period covers cold start. Upgrade to `condition: service_healthy` after confirming Compose spec version on Synology supports it.

### What Works

- [2026-04-30] All 12 managed stacks have README.md and .env.example in place — docs coverage is complete.
- [2026-04-30] All 12 stacks have per-stack INVENTORY.md and PROPOSAL.md under `docs/hive/proposals/<stack>/`; \_baseline and \_haproxy proposals also exist.
