...
- [2026-05-01] Docker Security Advisor warnings are expected for stacks that intentionally run with elevated privileges: root UID 0 for most services, `github-desktop` running with `seccomp:unconfined`, `no-new-privileges:true`, and `IPC_LOCK` for Electron/KasmVNC, and the optional Docker `zabbix-agent2` running privileged only when its service is uncommented; these are documented in `docs/hive/NAS_DEPLOYMENT.md` and relevant stack READMEs and should not be 'fixed' away.
