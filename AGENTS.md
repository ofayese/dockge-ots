...
- [2026-05-01] **Block 3 post-rework gates (healthcheck, paths, manifest, HIVE audit):** **`HEALTHCHECK POLICY`** — every stack must have a `healthcheck:` block **or** a documented `# No healthcheck:` reason in `compose.yaml`. Exemptions are listed in **`docs/hive/HEALTHCHECK_EXEMPTIONS.md`** (currently **mcp-tools-config** one-shot Busybox; **acme-sh** cron-style daemon, no HTTP/TCP probe). **holyclaude** uses a **TCP healthcheck** (`nc` on `127.0.0.1:3000`, `start_period: 90s`) — not exempt. **`DATABASES` stack:** manifest entry **`"databases:db"`** only; MariaDB and Postgres data both under **`db/`**; no separate app-level **`databases/data`** bind — intentional. **`HIVE_OBJECTIVE.md`:** stack names live in a **markdown table** (backtick list in the “Stack folders” row), not `-` bullets — parity vs `ls stacks/` uses the **table-aware** command in **`docs/hive/NAS_DEPLOYMENT.md`** (`grep "Stack folders"` + `grep -oE` on backtick names). **Path hygiene:** no bare **`dockge/stacks/`** in `stacks/` comments or examples without **`STACK_ROOT`** context (audit: `grep -rn "dockge/stacks/" stacks/ | grep -v "STACK_ROOT"`). **`github-desktop`:** manifest **`github-desktop:config`** (KasmVNC GUI — `/config` only); compose carries **`security_opt: seccomp:unconfined`** and **`cap_add: IPC_LOCK`** with inline comments for Electron/KasmVNC — Docker Security Advisor may flag; treat as documented in **`docs/hive/NAS_DEPLOYMENT.md`** (Security Advisor warnings) and **`stacks/github-desktop/README.md`**.
