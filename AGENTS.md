...
- **App state:** `scripts/dockge-start.sh` mounts the **repo root** (`DOCKGE_ROOT`, default `/volume1/docker/dockge`) at **`/app/data`** so Dockge’s DB/files live next to `stacks/` — **no** separate `.../dockge/data/` directory is required for **new** installs. **Upgrades from the old `.../data:/app/data` bind:** the script **one-time migrates** files from `${DOCKGE_ROOT}/data/` into `${DOCKGE_ROOT}/` when `data/dockge.db` exists and repo-root `dockge.db` is **missing or empty** (skips if root `dockge.db` is **non-empty**; replaces **0-byte** placeholders so a failed first boot does not block migration). Skips name clashes with `stacks`, `scripts`, `.git`. Run the updated script **before** `docker run` after `docker rm` so the DB is on disk where the new mounts expect it.
