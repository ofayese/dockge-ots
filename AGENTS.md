...
- [2026-05-02] **OTS/MFT namespaces (`401885b` + follow-up):** `*.ots.olutechsys.com` → OTS NAS (`traefik-ots`), `*.mft.olutechsys.com` → MFT NAS (`traefik-mft`); wildcard CNAMEs (DNS-only / grey cloud) — no per-service DNS. **Certs:** `acme-sh` issues **`ots-sub/`** (`*.ots.olutechsys.com`) and **`mft-sub/`** (`*.mft.olutechsys.com`) RSA 2048 DNS-01 via Cloudflare; Traefik consumes PEMs via **read-only** `${ACME_CERT_ROOT}/<ns>-sub` bind + `config/tls.yaml`. **Manifest:** `"traefik-ots:config"`, `"traefik-mft:config"`. **Docs:** `docs/hive/dns/olutechsys.com.zone`, `docs/hive/SERVICE_MAP.md`, `stacks/acme-sh/SETUP.md` (issue + install-cert). **Ops:** deploy Traefik after certs exist; services join `traefik-ots` / `traefik-mft` external bridge + Traefik labels — no DNS change for new hostnames. Zone reserves **`lab`/`dev`** (commented) until confirmed.
...
- [2026-05-01] **Docker Security Advisor — intentional rows to acknowledge** (do not remove settings from `compose.yaml` just to silence warnings): **`seccomp:unconfined`** on **github-desktop** (Electron/KasmVNC); **`IPC_LOCK`** on **github-desktop** (Electron memory locking); **`/var/run/docker.sock`** read-only on **traefik-ots** and **traefik-mft** (label discovery); **containers as root (UID 0)** fleet-wide (`PUID`/`PGID` default `0` by design); **`privileged: true`** on **zabbix-agent2** only when that service is uncommented. **`no-new-privileges:true` is not** an Advisor warning for **github-desktop** — that stack **omits** NNP on purpose (**`bfa07bd`**). See `docs/hive/NAS_DEPLOYMENT.md` → **Security Advisor warnings**; generalized **`no-new-privileges`** policy — **## Stack hardening defaults** above.
