...
Raise **all 26** Dockge stack folders under this repo to **production-grade reliability, observability, and maintainability** on Synology without changing service intent, and produce a **deployable HAProxy front-door spec** (stretch) using existing ACME material under `/volume1/certs/acme/`.
...
| Stack folders (Dockge `DOCKGE_STACKS_DIR`) | **26** — `acme-sh`, `agents_gateway_data`, `code-server`, `codex-docs`, `databases`, `docker-model-runner`, `dozzle`, `github-desktop`, `grafana-prom`, `holyclaude`, `homepage`, `it-tools`, `mcp-tools-config`, `ollama`, `openresume`, `portainer`, `psu-ots`, `rag-stack`, `remotely`, `searxng`, `synology-api-bridge`, `traefik-mft`, `traefik-ots`, `warp-main`, `watchtower`, `zabbix` |
| Workers (hive inventory)                   | **26** — **one worker per stack folder** (same names as rows above)                                                                                                                                                                                                                                                                                                     |
...
| TLS                                        | `acme-sh` issues to `/volume1/certs/acme/` — **do not edit cert/key files**; reference paths only in proposals. Operator reissue + Traefik + Google OAuth Step 1: **[docs/hive/CERT_REISSUE_TRAEFIK_OAUTH_RUNBOOK.md](docs/hive/CERT_REISSUE_TRAEFIK_OAUTH_RUNBOOK.md)**                                                                                                |
...
```text
Read HIVE_OBJECTIVE.md. 26 workers: 1 per stack folder. Proposals only in docs/hive/proposals/. Queen consensus before cross-stack or compose apply. Stretch: _haproxy/haproxy.cfg.
```
...
claude-flow hive-mind spawn --count 26 --claude -o 'Read HIVE_OBJECTIVE.md. 26 workers: 1 per stack folder. Proposals only in docs/hive/proposals/. Queen consensus before cross-stack or compose apply. Stretch: _haproxy/haproxy.cfg.'
```
...
```mermaid
flowchart TD
  Init[hive-mind init hierarchical-mesh] --> Spawn[spawn n26 claude short -o]
  Spawn --> Queen[Queen owns HIVE_OBJECTIVE.md]
  Queen --> Fan[26 workers 1 per stack]
