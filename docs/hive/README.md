...
- `COMPOSE_FILENAMES.md` — stacks that use `docker-compose.yml` / `docker-compose.yaml` instead of `compose.yaml`, plus non-Compose catalog files (for example `mcp-tools-config/docker-mcp.yaml`).
- **NAS operations** — see [`../../HIVE_OBJECTIVE.md`](../../HIVE_OBJECTIVE.md) section **NAS Deployment Notes** (clone/rsync workflow, `scripts/fix-permissions.sh`, UID defaults, docker.sock guidance).
