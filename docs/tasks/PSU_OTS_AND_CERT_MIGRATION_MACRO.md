...
- **`stacks/psu-ots/compose.yaml`** — digest-pinned `ironmansoftware/universal`, **`traefik-ots`** external network, **`${STACK_ROOT}/psu-ots/data:/data`**, **`${DOCKGE_REPO_ROOT}:/nas-repo:ro`**, **`${ACME_CERT_ROOT}:/certs/acme:ro`**, Traefik labels for **`psu.otsorundscore.olutechsys.com`**.
...
**TLS migration (already applied in tree):** Traefik stacks mount **`otsorundscore/`** and **`misfitsds/`** PEM dirs; update **live NAS** `acme.sh` orders and PEM paths before restarting Traefik.
...
**Phase 4** — ACME issued for **`otsorundscore/`** + **`misfitsds/`**, Traefik green, `verify-dns-views.sh --hairpin` (interpret per **`docs/hive/DNS_VIEWS_QUICK_REF.md`**).
