...
- **validate-haproxy-proposal.sh** — `haproxy -c` on `docs/hive/proposals/_haproxy/haproxy.cfg` when `haproxy` is in `PATH`; otherwise prints skip (use DSM to validate on-package installs).
