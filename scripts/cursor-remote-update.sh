...
deploy_to_remote() {
    ...
    ssh "$REMOTE_SSH_HOST" "echo 'SSH OK'" >/dev/null \
        && print_message green "  SSH connection OK" \
        || { print_message red "  SSH connection failed"; exit 1; }
    ...
    ssh "$REMOTE_SSH_HOST" "mkdir -p ${remote_install_dir}"

    # Check if already installed on remote
    if ssh "$REMOTE_SSH_HOST" "test -d ${remote_install_dir}/bin" 2>/dev/null; then
